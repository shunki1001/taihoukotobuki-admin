...
const SimpleBlogEditor: React.FC<SimpleBlogEditorProps> = ({ value, onChange }) => {
  ...
  const handleImageUpload = () => {
    ...
    // onChange(`${value}\n![画像ALTテキスト](${imageUrl})`);
    alert('画像アップロード機能は未実装です。Contentful側で画像をアップロードし、URLを貼り付けてください。');
  };
  ...
  return (
    <div className="border border-gray-300 dark:border-gray-600 rounded-md">
      <div className="flex items-center p-2 border-b border-gray-300 dark:border-gray-600 space-x-2">
        <Button size="sm" variant="secondary" onClick={handleImageUpload} title="画像を挿入">
          {/* <ImagePlus size={18} /> */}
          画像
        </Button>
        <Button size="sm" variant="secondary" onClick={() => setShowLinkModal(true)} title="リンクを挿入">
          {/* <LinkIcon size={18} /> */}
          リンク
        </Button>
      ...
      {showLinkModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md">
            <h3 className="text-lg font-medium mb-4 text-gray-900 dark:text-white">リンクを挿入</h3>
            <input
              type="url"
              value={linkUrl}
            ...
            <div className="flex justify-end space-x-2">
              <Button variant="secondary" onClick={() => setShowLinkModal(false)}>キャンセル</Button>
