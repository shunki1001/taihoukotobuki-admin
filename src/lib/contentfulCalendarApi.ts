...
export const createIrregularHour = async (data: Omit<IrregularHour, 'id'>): Promise<IrregularHour> => {
  try {
...
export const updateIrregularHour = async (id: string, data: Partial<Omit<IrregularHour, 'id'>>): Promise<IrregularHour | null> => {
  try {
    const space = await contentfulManagementClient.getSpace(spaceId);
    const environment = await space.getEnvironment('master');
